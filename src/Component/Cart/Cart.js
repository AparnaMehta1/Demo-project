import React , {useState} from 'react'
import "./Cart.css";
import {AiFillHeart} from "react-icons/ai";
import {MdDeleteOutline} from "react-icons/md"

const Cart  = () => {
    
    const   [num,setNum] = useState(1);
    const incNumber= ()=>{
        setNum(num+1)
       };
       const decNumber=()=>{
         if (num>0){
         setNum(num-1)}
         else{alert("Sorry, Zero Limit Reached") (setNum(0))} 
       }; 
  return (
    <>
    <h1>Cart</h1>
    <hr />
    <div className='row'>
        <div className='col-3'>
            <img className='Image' src='https://cdn.pixabay.com/photo/2016/12/06/09/31/blank-1886008__340.png'></img>
        </div>
       
        <div className='col-6 '>
            <p className='d-flex justify-content-left fw-bold text-muted'>Relaxed Fit T-shirt</p>
            <p className=' m-1 d-flex justify-content-left'> $ 12.99 <span className=' fw-bold text-success mx-2'>In Stock</span></p>

            <div class="btn-group" role="group" >
  <button type="button" class="btn btn-light d-flex justify-content-left" onClick={decNumber}>-</button>
  <button type="button" class="btn btn-light">{num}</button>
  <button type="button" class="btn btn-light" onClick={incNumber}>+</button>
</div>
        </div>

        <div className='col-3 '>
            <p className='text-muted  fw-bold'>$12.99</p>
        <div className='d-flex justify-content-center '>
        <p className='m-1 text-muted'><AiFillHeart />Save</p>
            <p className='m-1 text-muted'> <MdDeleteOutline />Delete</p>
        </div>
        </div>



    </div>
    <div className='row'>
        <div className='col-3'>
            <img className='image1' src='https://m.media-amazon.com/images/I/61RimBz8dOL._UX679_.jpg'></img>
        </div>
       
        <div className='col-6 '>
            <p className='d-flex justify-content-left fw-bold text-muted'>Nylon Sports Cap</p>
            <p className=' m-1 d-flex justify-content-left'> $ 14.99  <span>Avaliable in 2 days </span></p>
            <div class="btn-group" role="group d-flex justify-content-left" >
  <button type="button" class="btn btn-light" onClick={decNumber}>-</button>
  <button type="button" class="btn btn-light">{num}</button>
  <button type="button" class="btn btn-light" onClick={incNumber}>+</button>
</div>

        </div>

        <div className='col-3 '>
            <p className='text-muted  fw-bold'>$12.99</p>
        <div className='d-flex justify-content-center '>
        <p className='m-1 text-muted'><AiFillHeart />Save</p>
            <p className='m-1 text-muted'> <MdDeleteOutline />Delete</p>
        </div>
        </div>



    </div>
    <div className='row'>
        <div className='col-3'>
            <img className='image3' src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTOq8jHLv5T_iazRU2jqfu-ES_OuS4mRQF2p86BdGwH&s'></img>
        </div>
       
        <div className='col-6 '>
            <p className='d-flex justify-content-left fw-bold text-muted'>Snakers</p>
            <p className=' m-1 d-flex justify-content-left'> $ 34.99 <span className='mx-2 fw-bold text-danger'>Out of Stock</span></p>

            <div class="btn-group" role="group" >
  <button type="button" class="btn btn-light" onClick={decNumber}>-</button>
  <button type="button" class="btn btn-light">{num}</button>
  <button type="button" class="btn btn-light" onClick={incNumber}>+</button>
</div>
        </div>

        <div className='col-3 '>
            <p className='text-muted  fw-bold'>$34.99</p>
        <div className='d-flex justify-content-center '>
        <p className='m-1 text-muted'><AiFillHeart />Save</p>
            <p className='m-1 text-muted'> <MdDeleteOutline />Delete</p>
        </div>
        </div>



    </div>
    <div className='row'>
        <div className='col-3'>
            <img className='image4' src='data:image/jpeg;base64,'></img>
        </div>
       
        <div className='col-6 '>
            <p className='d-flex justify-content-left fw-bold text-muted'>Slim Fit Suit Vest</p>
            <p className=' m-1 d-flex justify-content-left'> $ 17.99 <span className=' fw-bold text-success'>In Stock</span></p>

            <div class="btn-group" role="group" >
  <button type="button" class="btn btn-light" onClick={decNumber}>-</button>
  <button type="button" class="btn btn-light">{num}</button>
  <button type="button" class="btn btn-light" onClick={incNumber}>+</button>
</div>
        </div>

        <div className='col-3 '>
            <p className='text-muted  fw-bold'>$17.99</p>
        <div className='d-flex justify-content-center '>
        <p className='m-1 text-muted'><AiFillHeart />Save</p>
            <p className='m-1 text-muted'> <MdDeleteOutline />Delete</p>
        </div>
        </div>



    </div>

    </>
  )
}

export default Cart;